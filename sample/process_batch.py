...
def process_single_vendor(zip_str, source, destination):
    ...
    for doc in [dI for dI in os.listdir(source) if os.path.isdir(os.path.join(source,dI))]:
        ...
        try:
            ...
            if os.path.exists(os.path.join(doc_destination, doc + '.json')):
                ...
                extract_key_value_pairs(doc, zip_str, doc_source, doc_destination)
        except:
            print(doc, " failed")
        # break
...
def process_folder_of_vendors(source, destination):
    for each_vendor in [dI for dI in os.listdir(source) if os.path.isdir(os.path.join(source,dI))]:
        print("vendor :", each_vendor)
        process_single_vendor('no', os.path.join(source, each_vendor), os.path.join(destination, each_vendor))
        break

if __name__ == '__main__':
    folder = os.path.join("C:\\", "Users", "JadaPrannoyNoelJada", "Documents", "Work", "Dow", "Results", "Specific vendor")
    source = os.path.join("C:\\", "Users", "JadaPrannoyNoelJada", "Documents", "Work", "Dow", "Results", "Specific vendor datacap")
    destination = os.path.join("C:\\", "Users", "JadaPrannoyNoelJada", "Documents", "Work", "Dow", "Results", "Specific vendor ricoh output")

    #split files into folders
    # ArrangeFilesintoFolder2(folder, source)

    #process folder which has multiple vendors OCR data
    process_folder_of_vendors(source, destination)
